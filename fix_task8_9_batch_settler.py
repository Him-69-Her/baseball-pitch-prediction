#!/usr/bin/env python3
"""
TINY-HUB — Tasks #8 + #9: Batch Settlement + Cloud Functions Gen 2
===================================================================
Combined rewrite of d91_settler.py:

Task #8 — Cloud Functions gen 2:
  • Pub/Sub push triggers trade ingestion (no polling loop)
  • Cloud Scheduler triggers hourly batch flush
  • Auto-restarts, auto-scales, no Docker container needed
  • Falls back to Docker container mode if not on Cloud Functions

Task #9 — Batch Settlement:
  • Trades buffer in memory (or Firestore for Cloud Functions)
  • Every BATCH_INTERVAL_SEC (default 3600 = 1 hour):
    - Aggregate net MWh per building
    - Push 1 bulk listResource + purchaseResource per building
    - Mint/burn THN in aggregate
  • Gas savings: ~95% reduction (1 tx/building/hour vs 1 tx/trade)

Architecture:
  Docker mode:  Pub/Sub subscriber → buffer → timer flush
  Cloud mode:   Pub/Sub push → buffer in Firestore → Scheduler → flush

Run from project root:
    python3 fix_task8_9_batch_settler.py

Creates:
    batch_settler.py          — new settler with batch aggregation
    deploy_cloud_function.sh  — Cloud Functions gen 2 deploy script
"""
from pathlib import Path

# ══════════════════════════════════════════════════════════════
# FILE 1: batch_settler.py — the new settler
# ══════════════════════════════════════════════════════════════

BATCH_SETTLER = Path("batch_settler.py")
BATCH_SETTLER.write_text('''"""
...
''', encoding="utf-8")
print("  ✅ batch_settler.py created")


# ══════════════════════════════════════════════════════════════
# FILE 2: deploy_cloud_function.sh
# ══════════════════════════════════════════════════════════════

DEPLOY_CF = Path("deploy_cloud_function.sh")
DEPLOY_CF.write_text('''#!/bin/bash
# ═══════════════════════════════════════════════════════════════
...
echo ""
''', encoding="utf-8")
print("  ✅ deploy_cloud_function.sh created")


# ══════════════════════════════════════════════════════════════
# PATCH docker-compose.yml — swap settler command
# ══════════════════════════════════════════════════════════════
DC = Path("docker-compose.yml")
if DC.exists():
    dc_src = DC.read_text(encoding="utf-8")

    OLD_SETTLER_CMD = "command: python3 -u d91_settler.py"
    NEW_SETTLER_CMD = "command: python3 -u batch_settler.py"

    if OLD_SETTLER_CMD in dc_src:
        dc_src = dc_src.replace(OLD_SETTLER_CMD, NEW_SETTLER_CMD, 1)
        DC.write_text(dc_src, encoding="utf-8")
        print("  ✅ docker-compose.yml: settler → batch_settler.py")
    elif "batch_settler" in dc_src:
        print("  ⏭️  docker-compose.yml: already using batch_settler")
    else:
        print("  ⚠️  docker-compose.yml: settler command not found")

    # Also add BATCH_INTERVAL_SEC env var
    if "BATCH_INTERVAL_SEC" not in dc_src:
        OLD_SETTLER_ENV = """    environment:
    ...
    depends_on:
      ...
      d63:
        ...
        if OLD_SETTLER_ENV in dc_src:
            dc_src = dc_src.replace(OLD_SETTLER_ENV, NEW_SETTLER_ENV, 1)
            DC.write_text(dc_src, encoding="utf-8")
            print("  ✅ docker-compose.yml: BATCH_INTERVAL_SEC added to settler")
else:
...
print()
print("  What changed:")
print("    • batch_settler.py — new settler with hourly batch aggregation")
print("    • deploy_cloud_function.sh — Cloud Functions gen 2 deploy script")
print("    • docker-compose.yml — settler now runs batch_settler.py")
print()
print("  Batch settlement:")
print("    • Trades buffer in memory per seller (station_id)")
print("    • Every hour: aggregate → 1 listResource + 1 purchaseResource per building")
print("    • ~95% gas savings vs per-trade settlement")
print("    • Bulk THN mint/burn per building")
print()
print("  Deploy (Docker mode — immediate):")
print("    sudo docker compose up -d --build settler")
print()
print("  Deploy (Cloud Functions — later):")
print("    chmod +x deploy_cloud_function.sh")
print("    ./deploy_cloud_function.sh")
print()
